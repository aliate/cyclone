...
func prepareSwiftConfig(loc Locations, srcContainer, dstContainer, prefix string, threads uint) (*objects.Configuration, []error) {
	util.Version = "cyclone"
	source := objects.SwiftLocation{
		AuthURL:                     loc.Src.AuthURL,
		UserName:                    loc.Src.Username,
		UserDomainName:              loc.Src.Domain,
		ProjectName:                 loc.Src.Project,
		ProjectDomainName:           loc.Src.Domain,
		Password:                    objects.AuthPassword(loc.Src.Password),
		ApplicationCredentialID:     loc.Src.ApplicationCredentialID,
		ApplicationCredentialName:   loc.Src.ApplicationCredentialName,
		ApplicationCredentialSecret: objects.AuthPassword(loc.Src.ApplicationCredentialSecret),
		RegionName:                  loc.Src.Region,
		ContainerName:               srcContainer,
		ObjectNamePrefix:            filepath.Dir(prefix),
	}

	target := objects.SwiftLocation{
		AuthURL:                     loc.Dst.AuthURL,
		UserName:                    loc.Dst.Username,
		UserDomainName:              loc.Dst.Domain,
		ProjectName:                 loc.Dst.Project,
		ProjectDomainName:           loc.Dst.Domain,
		Password:                    objects.AuthPassword(loc.Dst.Password),
		ApplicationCredentialID:     loc.Dst.ApplicationCredentialID,
		ApplicationCredentialName:   loc.Dst.ApplicationCredentialName,
		ApplicationCredentialSecret: objects.AuthPassword(loc.Dst.ApplicationCredentialSecret),
		RegionName:                  loc.Dst.Region,
	...
	config := &objects.Configuration{
		Swift: target,
		JobConfigs: []objects.JobConfiguration{
			{
				Source: objects.SourceUnmarshaler{
					Source: &source,
				},
				Target:         &target,
				IncludePattern: fmt.Sprintf("%s.*", filepath.Base(prefix)),
			},
		},
	}

	config.WorkerCounts.Transfer = threads
	config.Swift.ValidateIgnoreEmptyContainer = true
	// TODO: fail, when target file exists

	errors := config.Swift.Validate("swift")
	for idx, jobConfig := range config.JobConfigs {
...
func cloneBackup(srcVolumeClient, dstVolumeClient *gophercloud.ServiceClient, srcBackup *backups.Backup, toBackupName string, toContainerName string, threads uint, loc Locations) (*backups.Backup, error) {
	...
	// TODO: recursive
	config, errors := prepareSwiftConfig(loc, *backupRecord.Container, toContainerName, *backupRecord.ServiceMetadata, threads)
	if len(errors) > 0 {
...
var BackupCloneCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		dstBackup, err := cloneBackup(srcVolumeClient, dstVolumeClient, srcBackup, toName, toContainerName, threads, loc)
		if err != nil {
			return err
		}
